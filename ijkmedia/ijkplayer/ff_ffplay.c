...
//    AVIOContext是FFMPEG管理输入输出数据的结构体
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        //如果type 不是unknown 并且 是 ffplayer的stream 并且 这个type的index 是-1
        if (type >= 0 && ffp->wanted_stream_spec[type] && st_index[type] == -1)
            //判断st是否包含ic流在wanted_stream_spec列表里面
    ...
    //如果video流数大于1并且st_index[AVMEDIA_TYPE_VIDEO]流少于0
    //报错
    if (video_stream_count > 1 && st_index[AVMEDIA_TYPE_VIDEO] < 0) {
        st_index[AVMEDIA_TYPE_VIDEO] = first_h264_stream;
        av_log(NULL, AV_LOG_WARNING, "multiple video stream found, prefer first h264 stream: %d\n", first_h264_stream);
    }
    //如果ffplayer的video_disable不为真
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name)
{
    ...
    //初始化videostate后赋值给ffplayer的is属性
    ffp->is = is;
    //从file_name复制字段到ffp->input_filename

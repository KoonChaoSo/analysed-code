...
//block 的结构体 和 系统一样的结构体，方便从block转换成方法
...
#pragma mark - 核心1！！
//!!!核心 1、创建子类。2、处理实现过的MsgForwardIMP 的类或者对象。3、将需要替换的selector 指向_OBJC_MSGFORWARD
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    //是否指向_objc_msgForward_stret/_objc_msgForward  是否为消息转发
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        ...
        //使用forwardInvocation来统一处理。将原来的SEL指向到原来的SEL找不到方法的forwardInovation
        //替换之后调用原来的方法的时候，就会调用指向的forwardInvocation，即是传说中的消息转发
...
static void aspect_swizzleForwardInvocation(Class klass) {
    ...
    //用__ASPECTS_ARE_BEING_CALLED__  替换  klass的forwardInvocation的selector 的IMP，并且定义返回值。返回值是原来实现的方法
    //替换消息转发
    IMP originalImplementation = class_replaceMethod(klass, @selector(forwardInvocation:), (IMP)__ASPECTS_ARE_BEING_CALLED__, "v@:@");
    if (originalImplementation) {
        //如果没有原来实现的方法那就添加自定义的方法。
...
#pragma mark - 核心1！！
//！！！！！核心
//当invocation 调用到 forwardInvoction: 就会调用到这里 666666
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    //如果没有hooks 的话~~返回原来的流程~~6666666666，给一百万个赞
    if (!respondsToAlias) {
...
- (BOOL)invokeWithInfo:(id<AspectInfo>)info {
	...
	void *argBuf = NULL;
    //为什么是2
    //这个需要了解nsinvocation的methodSignature的结构
